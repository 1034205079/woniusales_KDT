import sys, traceback, time, html, yagmail, os, glob
...
class Runner(object):
    ...
    def send_mail(self):
        ...
        yag.send(to=Config.receive_mail,  # 收件人
                 subject=f"自动化测试报告{self.report_time}",  # 主题
                 contents=f"{self.summary_message} \n 报告服务器地址：http://192.168.12.13:9999/result",  # 正文
                 attachments=[os.path.join(Config.project_path, "log/mylog.log")]  # 附件
                 )
        yag.close()
